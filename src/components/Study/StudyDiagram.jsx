...
export default function StudyDiagram({ diagram }) {
  ...
  return (
    <figure
      className="my-3.5 rounded-xl border border-[var(--border-beige)] bg-[var(--bg-card)] overflow-hidden shadow-sm"
      aria-label={title}
      style={{
        margin: '12px 0',
        borderRadius: '12px',
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
          ...
          <div>
            <figcaption style={{ fontSize: '13px', fontWeight: 800, color: 'var(--text-charcoal)', margin: 0 }}>
              {title}
            </figcaption>
            {purpose && (
          ...
          {hasSteps && (
            <button
              ...
              style={{
                ...
                padding: '3px 9px',
                borderRadius: '9999px',
                backgroundColor: showSteps ? '#C85A32' : 'var(--bg-card)',
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      justifyContent: 'center',
                      borderRadius: '8px',
                      backgroundColor: isActive ? 'rgba(200, 90, 50, 0.12)' : 'var(--bg-card)',
                      border: `1.5px solid ${isActive ? 'var(--accent-terracotta)' : 'var(--border-beige)'}`,
                  ...
                  >
                    <span style={{
                      ...
                      fontSize: '14px',
                      fontWeight: 800,
                      color: isActive ? 'var(--accent-terracotta)' : 'var(--text-charcoal)'
                    }}>
                      {el.label}
                    </span>
                    {el.sublabel && (
            ...
            {currentStep?.pointerState && (
              <div style={{
                ...
                {Object.entries(currentStep.pointerState).map(([ptrKey, ptrVal]) => (
                  ...
                  >
                    <span style={{ textTransform: 'uppercase', color: 'var(--accent-terracotta)' }}>{ptrKey}:</span> {ptrVal}
                  </span>
                ))}
              </div>
            )}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center' }}>
                {connections.map((conn, idx) => (
                  ...
                  >
                    ...
                    {conn.label && <span style={{ color: 'var(--text-charcoal)', fontStyle: 'italic' }}>({conn.label})</span>}
                  </div>
                ))}
              </div>
            )}
                ...
                <React.Fragment key={el.id || idx}>
                  <div
                    style={{
                      ...
                      padding: '8px 12px',
                      borderRadius: '8px',
                      backgroundColor: isActive ? 'rgba(200, 90, 50, 0.1)' : 'var(--bg-card)',
                      border: `1.5px solid ${isActive ? 'var(--accent-terracotta)' : 'var(--border-beige)'}`,
                      textAlign: 'center',
                      boxShadow: isActive ? '0 0 8px rgba(200, 90, 50, 0.2)' : '0 1px 3px rgba(35, 25, 15, 0.04)'
                    }}
                  >
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '13px', fontWeight: 800, color: 'var(--text-charcoal)' }}>
                      {el.label}
                    </div>
                    {el.sublabel && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxWidth: '440px', margin: '0 auto' }}>
            {connections.length > 0 ? (
              connections.map((conn, idx) => (
                ...
                >
                  <span style={{ fontWeight: 800, color: 'var(--text-charcoal)' }}>{conn.from}</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--accent-terracotta)', fontWeight: 700 }}>
                    <span>{conn.label || '─►'}</span>
                    <ArrowRight size={13} />
                  </div>
                  <span style={{ fontWeight: 800, color: 'var(--text-charcoal)' }}>{conn.to}</span>
                </div>
              ))
            ) : (
              elements.map((el, idx) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '8px 12px',
                    borderRadius: '8px',
                    backgroundColor: activeElementIds.has(el.id) ? 'rgba(200, 90, 50, 0.1)' : 'var(--bg-card-subtle)',
                    border: `1px solid ${activeElementIds.has(el.id) ? 'var(--accent-terracotta)' : 'var(--border-beige)'}`
                  }}
                >
                  <span style={{
                    width: '20px',
                  ...
                  <div>
                    <div style={{ fontSize: '12.5px', fontWeight: 700, color: 'var(--text-charcoal)' }}>{el.label}</div>
                    {el.sublabel && <div style={{ fontSize: '10.5px', color: 'var(--text-muted)' }}>{el.sublabel}</div>}
                  </div>
                </div>
              ))
            )}
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '12px',
                    borderRadius: '8px',
                    backgroundColor: isActive ? 'rgba(200, 90, 50, 0.08)' : 'var(--bg-card-subtle)',
                    border: `1.5px solid ${isActive ? 'var(--accent-terracotta)' : 'var(--border-beige)'}`,
                    textAlign: 'center',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: '4px',
                  border: '1px solid var(--border-beige)',
                  backgroundColor: currentStepIdx === 0 ? 'var(--bg-card-subtle)' : 'var(--bg-card)',
                  color: currentStepIdx === 0 ? 'var(--text-muted)' : 'var(--text-charcoal)',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
      ...
      {description && (
        <div style={{
          ...
          <strong style={{ color: 'var(--text-charcoal)' }}>Visual summary: </strong>
          {description}
        </div>
      )}
    </figure>
